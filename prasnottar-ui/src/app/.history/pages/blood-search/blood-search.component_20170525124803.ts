...
export class BloodSearchComponent implements OnInit {
  ...
  constructor(
    ...
    this._route.queryParams.subscribe((params: Params) => {
      ...
      if (!this.bloodGroup) {
        this._router.navigateByUrl("/raktadan");
      }
      if (this.location) { this.onBloodSearch(this.bloodGroup, this.location); }
      else { this.onBloodSearchOnly(this.bloodGroup) };
    });
  }
  ngOnInit() {
  ...
  onBloodSearchOnly(bloodgroup) {
    this.genericService.getAllResources(this.BASE_URL + "blooddonar/search/" + bloodgroup)
      .subscribe(
      res => {
        this.response = <Response>res;
        if (this.pre.checkResponseValid(this.response)) {
